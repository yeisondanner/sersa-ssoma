<?php
class ProcessModel extends Mysql
{
    private string $name;
     ...
     * @return array
     */
    public function select_process_inner_macroprocess(): array
    {
        $query = "SELECT
                        tbmp.mp_name,
                        tbp.*
                    FROM
                        tb_macroprocess AS tbmp
                        INNER JOIN tb_process AS tbp ON tbp.macroprocess_id = tbmp.idMacroprocess
                    ORDER BY
                        tbp.idProcess DESC;";
